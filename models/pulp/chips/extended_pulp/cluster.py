...
from pulp.chips.extended_pulp.l1_subsystem import L1_subsystem
...
class Cluster(st.Component):
    ...
    def __init__(self, parent, name, config_file, cid: int=0):
        ...
        ima_irq_0           = self.get_property('pe/irq').index('acc_2')
        ima_irq_1           = self.get_property('pe/irq').index('acc_3')
        ...
        # MCHAN
        mchan = Mchan(self, 'dma', nb_channels=nb_pe+1, max_burst_length=2048, nb_loc_ports=32)
        ...
        cluster_ico.add_mapping('periph_ico', **self._reloc_mapping(self.get_property('peripherals/mapping')))
        self.bind(cluster_ico, 'periph_ico', periph_ico, 'input')

        cluster_ico.add_mapping('periph_ico_alias', **self.get_property('peripherals/alias'), add_offset=hex(int(self.get_property('mapping/base'), 0) + (int(self.get_property('mapping/size'), 0)) * cid))
        ...
        for i in range(0, mchan.get_property('nb_loc_ports')):
        ...
        # IMA
        for i in range(0, ima.get_property('nb_masters')):
            self.bind(ima, 'out_%d' % i, l1, 'ima_in_%d' % i)

        for i in range(0, nb_pe):
            self.bind(ima, 'irq_0', event_unit, 'in_event_%d_pe_%d' % (ima_irq_0, i))
            self.bind(ima, 'irq_1', event_unit, 'in_event_%d_pe_%d' % (ima_irq_1, i))

        try:
            f = open(os.path.join(eval(tp.get_child_str('peripherals/ima/preload_path')), 'IMA%d_preload.bin' % cid))
            ima.add_properties('preload_file', os.path.join(eval(tp.get_child_str('cluster/peripherals/ima/preload_path')), 'IMA%d_preload.bin' % cid))
            f.close()
        except:
            pass
