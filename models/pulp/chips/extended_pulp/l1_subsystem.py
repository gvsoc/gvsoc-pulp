...
class L1_subsystem(st.Component):
    ...
    def __init__(self, parent, name, cluster):
        ...
        ima_masters   = cluster.get_component('ima').get_property('nb_masters')
        dma_loc_ports = cluster.get_component('dma').get_property('nb_loc_ports')
        ...
        l1_interleaver_nb_masters = nb_pe + dma_loc_ports + ima_masters # 1 port per PE + N for DMA + N for IMA
        ...
        # EXT2LOC
        ext2loc = Converter(self, 'ext2loc', output_width=4, output_align=4, master_ports=32)

        # EXT2LOC test-and-set
        ext2loc_ts = Converter(self, 'ext2loc_ts', output_width=4, output_align=4, master_ports=32)
        ...
        for i in range(0, dma_loc_ports):
            self.bind(self, 'dma_in_%d' % i, interleaver, 'in_%d' % (nb_pe + i))

        for i in range(0, ima_masters):
            self.bind(self, 'ima_in_%d' % i, interleaver, 'in_%d' % (nb_pe + dma_loc_ports + i))

        # EXT2LOC
        self.bind(self, 'ext2loc', ext2loc, 'input')
        for i in range(0, ext2loc.get_property('master_ports')):
            self.bind(ext2loc, 'out_%d' % i, interleaver, 'in_%d' % (nb_pe + i))

        # EXT2LOC test-and-set
        self.bind(self, 'ext2loc_ts', ext2loc_ts, 'input')
        for i in range(0, ext2loc_ts.get_property('master_ports')):
            self.bind(ext2loc_ts, 'out_%d' % i, interleaver, 'ts_in_%d' % (nb_pe + i))
