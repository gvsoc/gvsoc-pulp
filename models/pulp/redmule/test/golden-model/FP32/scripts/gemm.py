...
if torch.cuda.is_available():
  X = torch.rand(m_size, n_size).float().cuda()
  W = torch.rand(n_size, k_size).float().cuda()
  Y = torch.rand(m_size, k_size).float().cuda()
  Z = torch.rand(m_size, k_size).float().cuda()
else:
...
for i in range(m_size):
    for j in range (n_size):
        x_bin = bin(np.float32(X[i][j]).view(np.int32))[2:].zfill(32)
...
for i in range(n_size):
    for j in range (k_size):
        w_bin = bin(np.float32(W[i][j]).view(np.int32))[2:].zfill(32)
...
for i in range(m_size):
    for j in range (k_size):
        y_bin = bin(np.float32(Y[i][j]).view(np.int32))[2:].zfill(32)
...
for i in range(m_size):
    for j in range (k_size):
        z_bin = bin(np.float32(Z[i][j]).view(np.int32))[2:].zfill(32)
...
while i < ZFlattened.size(dim = -1) - 1:
  c_bin_0 = bin(np.float32(ZFlattened[i]).view(np.int32))[2:].zfill(32)
  c_bin_1 = bin(np.float32(ZFlattened[i+1]).view(np.int32))[2:].zfill(32)
