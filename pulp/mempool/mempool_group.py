...
class Group(st.Component):
    ...
    def __init__(self, parent, name, parser, group_id: int=0, nb_cores_per_tile: int=4, nb_groups: int=4, total_cores: int= 256, bank_factor: int=4):
        ...
        for i in range(0, nb_tiles_per_group):
            tile_list.append(Tile(self,f'tile_{i}',parser=parser,tile_id=i, group_id=group_id, nb_cores_per_tile=nb_cores_per_tile, 
        ...
        for i in range(0, nb_tiles_per_group):
            self.bind(tile_list[i], 'grp_local_master_out', group_local_interleaver, 'in_%d' % i)

        #Group local interconnect -> Tile local slave
        for i in range(0, nb_tiles_per_group):
            self.bind(group_local_interleaver, 'out_%d' % i, tile_list[i], 'grp_local_slave_in')

        #Tile remote master -> Group remote routers
        for port in range(0, nb_remote_ports):
            for i in range(0, nb_tiles_per_group):
                self.bind(tile_list[i], f'grp_remt{port}_master_out', group_remote_master_interfaces[port], 'in_%d' % i)

        #Tile rom -> rom router
        for i in range(0, nb_tiles_per_group):
            self.bind(tile_list[i], 'rom', rom_router, 'input')

        #Tile l2 data -> l2 router
        for i in range(0, nb_tiles_per_group):
            self.bind(tile_list[i], 'L2_data', l2_router, 'input')
        ...
        for port in range(0, nb_remote_ports):
            for i in range(0, nb_tiles_per_group):
                self.bind(self, f'grp_remt{port}_tile{i}_slave_in', tile_list[i], f'grp_remt{port}_slave_in')
