...
void IDmaBeAxi::send_read_burst_to_axi()
{
    ...
    else if (status == vp::IoReqStatus::IO_REQ_DENIED)
    {
        this->stalled = true;
    }
    else
...
void IDmaBeAxi::read_handle_req_end(vp::IoReq *req)
{
    ...
    *req->arg_get(1) = (void *)1;  // Mark as responded
...
void IDmaBeAxi::fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    if (_this->pending_bursts.size() > 0 && !_this->pending_bursts.front()->get_is_write() && !_this->stalled)
    {
    ...
    if (_this->read_waiting_bursts.size() != 0 &&
        *_this->read_waiting_bursts.front()->arg_get(1) == (void *)1 &&
