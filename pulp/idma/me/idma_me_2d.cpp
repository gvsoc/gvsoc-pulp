    ...
    me_state(*this, "me_state", 32, true, ME_IDLE)
{
    ...
    if (this->gather_enable)
    {
        ...
        // A denied request is retained; grant permits progress downstream,
        // while the response callback supplies the index data.
...
void IDmaMe2D::fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    if (_this->transfer_queue.size() > 0 && _this->current_transfer == NULL)
    {
        ...
        if ((_this->current_transfer->config & 6) == 0)
        {
            _this->current_reps = 1;
        }

        _this->me_state.set(ME_DECOMPOSING);
        if (_this->current_transfer->config & 4)
        {
            ...
            return; // GIDLE -> GRUN is a registered transition in the RTL.
        }
    }

    if (_this->current_transfer && (_this->current_transfer->config & 4))
    {
        _this->gather_step();
        return;
    }
