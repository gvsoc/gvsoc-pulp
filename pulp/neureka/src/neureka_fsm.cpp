...
const char *neureka_state_to_string(NeurekaState state)
{
  switch (state) {
    case IDLE: return "IDLE";
    case JOB_START: return "JOB_START";
    case TILE_PREPARE: return "TILE_PREPARE";
    case LOAD_PREPARE: return "LOAD_OVERHEAD";
    case MATRIXVEC_PREPARE: return "MATRIXVEC_OVERHEAD";
    case ACCUM_STREAMIN: return "ACCUM_STREAMIN";
    case ACTIVATION_LOAD: return "ACTIVATION_LOAD";
    case MATRIX_VECTOR_COMPUTE: return "MATRIX_VECTOR_COMPUTE";
    case NORM_SHIFT: return "NORM_SHIFT";
    case NORM_MULTIPLY: return "NORM_MULTIPLY";
    case NORM_BIAS_SHIFT: return "NORM_BIAS_SHIFT";
    case OUTPUT_STREAMOUT: return "OUTPUT_STREAMOUT";
...
void Neureka::fsm_start_handler(vp::Block *__this, vp::ClockEvent *event) {
  ...
  _this->regfile_cxt();
  _this->job_running = 1;
  _this->state_phase_track_state = JOB_START;
  _this->state_phase_track_cycles = 0;
  _this->state_phase_track_valid = false;
  _this->state_phase_track_start_cycle = _this->clock.get_cycles();
...
void Neureka::fsm_handler(vp::Block *__this, vp::ClockEvent *event) {
  Neureka *_this = (Neureka *)__this;
...
void Neureka::fsm_end_handler(vp::Block *__this, vp::ClockEvent *event) {
  Neureka *_this = (Neureka *)__this;
  ...
  const int total_cycles = _this->end_cycles - _this->start_cycles;
  if (trace_state_phase_summary_only(_this->trace_level) && _this->state_phase_track_valid && _this->state_phase_track_cycles > 0) {
    const int64_t phase_end_cycle = _this->end_cycles;
    const int64_t phase_start_cycle = _this->state_phase_track_start_cycle;
    const int64_t phase_diff = phase_end_cycle - phase_start_cycle;
    _this->trace.msg(
      vp::Trace::LEVEL_INFO,
      "  %-22s  %11lld  %11lld  %10lld  %-22s\n",
      neureka_state_to_string(_this->state_phase_track_state),
      static_cast<long long>(phase_start_cycle),
      static_cast<long long>(phase_end_cycle),
      static_cast<long long>(phase_diff),
      neureka_state_to_string(IDLE)
    );
  }
  ...
  _this->state_phase_track_valid = false;
  _this->state_phase_track_cycles = 0;
  _this->state_phase_track_state = IDLE;
  _this->state_phase_track_start_cycle = 0;
}
...
void Neureka::fsm_loop() {
  ...
  if(state.get() == JOB_END && !this->fsm_end_event->is_enqueued()) {
...
int Neureka::fsm() {
  auto state_cur = this->state.get();
  auto state_next = state_cur;
  auto latency = 0;
  auto k_in_major_lim = 0;
  auto trace_major_indices = [this]() {
  ...
  // Baseline: disable all optional traces.
  ...
  switch(state_cur) {
    ...
    case JOB_START:
      ...
      state_next = TILE_PREPARE;
      break;
    
    case TILE_PREPARE:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State TILE_PREPARE\n");
      }
      this->constant_setup();
      if(this->streamin) {
        this->streamin_setup();
        state_next = ACCUM_STREAMIN;
      }
      else {
        state_next = LOAD_PREPARE;
      }
      if(this->fsm_traces) {
        trace_major_indices();
      }
    ...
    case ACCUM_STREAMIN:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State ACCUM_STREAMIN\n");
      ...
      if(this->streamin_exit_idx()) {
        ...
        if(this->accum_traces_poststreamin) {
          this->trace.msg(vp::Trace::LEVEL_DEBUG, "State ACCUM_STREAMIN Finished\n");
          this->debug_accum();
        }
        state_next = LOAD_PREPARE;
      }
    ...
    case LOAD_PREPARE:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State LOAD_PREPARE\n");
      }
      this->load_setup();
      if(this->fsm_traces) {
    ...
    case ACTIVATION_LOAD:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State ACTIVATION_LOAD\n");
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "  load_fbuf_lim=%d\n", this->load_fbuf_lim);
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "  load_i_fbuf_lim=%d\n", this->load_i_fbuf_lim);
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "  load_j_fbuf_lim=%d\n", this->load_j_fbuf_lim);
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "  load_k_in_lim=%d\n", this->load_k_in_lim);
    ...
    case MATRIXVEC_PREPARE:
      ...
      state_next = MATRIX_VECTOR_COMPUTE;
    ...
    case MATRIX_VECTOR_COMPUTE:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State MATRIX_VECTOR_COMPUTE\n");
    ...
    case NORM_MULTIPLY:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State NORM_MULTIPLY\n");
    ...
    case NORM_BIAS_SHIFT:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State NORM_BIAS_SHIFT\n");
    ...
    case OUTPUT_STREAMOUT:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State OUTPUT_STREAMOUT\n");
    ...
    case JOB_END:
      if(this->fsm_traces) {
        this->trace.msg(vp::Trace::LEVEL_DEBUG, "State JOB_END\n");
      }
      latency = 1;
      break;
  ...
  if (trace_state_phase_summary_only(this->trace_level)) {
    const int64_t current_cycle = this->clock.get_cycles();
    if (!this->state_phase_track_valid || this->state_phase_track_state != state_cur) {
      this->state_phase_track_state = static_cast<NeurekaState>(state_cur);
      this->state_phase_track_cycles = 0;
      this->state_phase_track_valid = true;
      this->state_phase_track_start_cycle = current_cycle;
    }

    this->state_phase_track_cycles += latency;

    if (state_next != state_cur) {
      const int64_t phase_end_cycle = current_cycle + latency;
      const int64_t phase_start_cycle = this->state_phase_track_start_cycle;
      const int64_t phase_diff = phase_end_cycle - phase_start_cycle;
      this->trace.msg(
        vp::Trace::LEVEL_INFO,
        "  %-22s  %11lld  %11lld  %10lld  %-22s\n",
        neureka_state_to_string(this->state_phase_track_state),
        static_cast<long long>(phase_start_cycle),
        static_cast<long long>(phase_end_cycle),
        static_cast<long long>(phase_diff),
        neureka_state_to_string(static_cast<NeurekaState>(state_next))
      );
      this->state_phase_track_state = static_cast<NeurekaState>(state_next);
      this->state_phase_track_cycles = 0;
      this->state_phase_track_start_cycle = phase_end_cycle;
    }
  }
