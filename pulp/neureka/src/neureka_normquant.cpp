...
bool Neureka::NormQuantBiasExecute(int& latency){
  ...
  std::array<InFeatType,32>  normquant_bias_32bits={}; 
  ...
  int index=0;
  for(int i=0; i<width; i=i+4){
    normquant_bias_32bits[index] = (normquant_bias_8bits[i+3]<<24) + (normquant_bias_8bits[i+2]<<16) + (normquant_bias_8bits[i+1]<<8) + normquant_bias_8bits[i];
    index++;
  }
