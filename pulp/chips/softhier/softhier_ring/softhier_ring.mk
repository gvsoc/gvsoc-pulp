...
config_file_ring ?= "pulp/pulp/chips/softhier/softhier_ring/softhier_arch.py"
ifdef cfg
	config_file_ring = "$(cfg)"
endif

sh-config:
	@if [ "$(config_file_ring)" != "pulp/pulp/chips/softhier/softhier_ring/softhier_arch.py" ]; then \
		cp -f $(config_file_ring) pulp/pulp/chips/softhier/softhier_ring/softhier_arch.py; \
	fi
	python3 pulp/pulp/chips/softhier/softhier_ring/utils/config.py $(config_file_ring)

sh-hw:
	make sh-config
	make TARGETS=pulp.chips.softhier.softhier_ring.softhier_target all
...
sh-sw:
	...
	cd sw_build && $(CMAKE) $(sw_cmake_arg) $(arch_cmake_arg) ../pulp/pulp/chips/softhier/softhier_ring/sw/ && make
...
sh-run:
	./install/bin/gvsoc --target=pulp.chips.softhier.softhier_ring.softhier_target --binary sw_build/softhier.elf run