...
class ClusterICache(gvsoc.systree.Component):

    def __init__(self, parent: gvsoc.systree.Component,
                name: str,
                nb_cores: int=0,
                has_cc: int=0,
                l1_line_size_bits: int=5,
                l1_nb_sets_bits: int=4,
                l1_nb_ways_bits: int=4):

        super(ClusterICache, self).__init__(parent, name)
        ...
        for i in range(0, nb_l1_banks):
            l1_caches.append(Cache(self, 'l1_bank%d' % i, nb_sets_bits=l1_nb_sets_bits, nb_ways_bits=l1_nb_ways_bits, line_size_bits=l1_line_size_bits, refill_latency=2, refill_shift=nb_l1_banks_log2, add_offset=i*l1_cache_line_size, enabled=True))
