...
class SoftHierSystem(gvsoc.systree.Component):
    ...
    def __init__(self, parent, name, parser, topology):
        ...
        # Control register
        virtual_interco.o_MAP(softhier_ctrl.i_INPUT(), base=arch.soc_register_base,
        ...
        for cluster_id in range(arch.num_cluster):
            ...
            noc.get_property('mappings')[f"wide_{wide_name}"] = {
                'base': wide_base, 'size': arch.cluster_tcdm_size, 'node_id': ni_node_id,
                'remove_offset': wide_base
            }
