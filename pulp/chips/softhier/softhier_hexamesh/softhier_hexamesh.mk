...
config_file_hexamesh ?= "pulp/pulp/chips/softhier/softhier_hexamesh/softhier_arch.py"
ifdef cfg
	config_file_hexamesh = "$(cfg)"
endif

sh-config:
	@if [ "$(config_file_hexamesh)" != "pulp/pulp/chips/softhier/softhier_hexamesh/softhier_arch.py" ]; then \
		cp -f $(config_file_hexamesh) pulp/pulp/chips/softhier/softhier_hexamesh/softhier_arch.py; \
	fi
	python3 pulp/pulp/chips/softhier/softhier_hexamesh/utils/config.py $(config_file_hexamesh)

sh-hw:
	make sh-config
	make TARGETS=pulp.chips.softhier.softhier_hexamesh.softhier_target all
...
sh-sw:
	...
	cd sw_build && $(CMAKE) $(sw_cmake_arg) $(arch_cmake_arg) ../pulp/pulp/chips/softhier/softhier_hexamesh/sw/ && make
...
sh-run:
	./install/bin/gvsoc --target=pulp.chips.softhier.softhier_hexamesh.softhier_target --binary sw_build/softhier.elf run