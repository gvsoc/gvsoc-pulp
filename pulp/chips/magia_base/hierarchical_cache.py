...
class Hierarchical_cache(gvsoc.systree.Component):
    ...
    def __init__(self, parent: gvsoc.systree.Component, name: str, nb_cores: int=0, has_cc: int=0, l1_line_size_bits: int=0):
        ...
        for i in range(0, nb_pes):
            l0_caches.append(Cache(self, 'l0_bank%d' % i, nb_sets_bits=0, nb_ways_bits=0, line_size_bits=7, refill_latency=0, enabled=True))

        # L1 caches
        l1_caches = []
        for i in range(0, nb_l1_banks):
            l1_caches.append(Cache(self, 'l1_bank%d' % i, nb_sets_bits=7, nb_ways_bits=1, line_size_bits=7, refill_latency=2, refill_shift=nb_l1_banks_log2, add_offset=i*l1_cache_line_size, enabled=True))
