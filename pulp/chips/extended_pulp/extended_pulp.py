...
from pulp.chips.extended_pulp.soc import Soc
from pulp.chips.extended_pulp.cluster import Cluster, get_cluster_name
...
class Extended_pulp(st.Component):

    def __init__(self, parent, name, parser, soc_config_file='pulp/chips/extended_pulp/soc.json', cluster_config_file='pulp/chips/extended_pulp/cluster.json', padframe_config_file='pulp/chips/extended_pulp/padframe.json'):
        super(Extended_pulp, self).__init__(parent, name)

        #
        # Properties
        #

        pulp_config_file = 'pulp/chips/extended_pulp/extended_pulp.json'

        self.add_properties(self.load_property_file(pulp_config_file))
        ...
        nb_cluster = self.get_property('nb_cluster', int)

        has_noc = self.get_property('hierarchical_interconnect/has_noc', bool)
        ...
        # Soc
        soc = Soc(self, 'soc', parser=parser, config_file=soc_config_file, chip=self, cluster=clusters[0])
        ...
        for cid in range(0, nb_cluster):
            ...
            if has_noc:
                self.bind(clusters[cid], 'soc', soc, 'soc_input_%d' % cid)
            else:
        ...
        if has_noc:
          self.bind(axi_proxy, 'out', soc, 'soc_input_0')
        else:
