...
class SoftHierSystem(gvsoc.systree.Component):
    ...
    def __init__(self, parent, name, parser):
       ...
       # --- FlooNoC Flex Initialization & Topology Building ---
        ...
        # Add links (Router <-> Router Torus Network)
        for z in range(dim_z):
            for y in range(dim_y):
                for x in range(dim_x):
                    r_id = routers_map[(x, y, z)]
                    
                    # Link East (X-axis) & Wraparound
                    if x < dim_x - 1:
                        east_id = routers_map[(x + 1, y, z)]
                        noc.add_link(r_id, east_id, latency=1)
                    elif x == dim_x - 1:
                        wrap_x_id = routers_map[(0, y, z)]
                        noc.add_link(r_id, wrap_x_id, latency=1)
                        
                    # Link South (Y-axis) & Wraparound
                    if y < dim_y - 1:
                        south_id = routers_map[(x, y + 1, z)]
                        noc.add_link(r_id, south_id, latency=1)
                    elif y == dim_y - 1:
                        wrap_y_id = routers_map[(x, 0, z)]
                        noc.add_link(r_id, wrap_y_id, latency=1)
                        
                    # Link Down (Z-axis) & Wraparound
                    if z < dim_z - 1:
                        down_id = routers_map[(x, y, z + 1)]
                        noc.add_link(r_id, down_id, latency=1)
                    elif z == dim_z - 1:
                        wrap_z_id = routers_map[(x, y, 0)]
                        noc.add_link(r_id, wrap_z_id, latency=1)

        # Generate routing tables
        # Could either be changed to import from json or generated by floogen
        # In any case, should be a function call to floonoc_flex so chip doesn't change
