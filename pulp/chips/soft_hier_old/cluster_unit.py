...
import gvsoc.runner
import pulp.chips.soft_hier_old.snitch.snitch_core as iss
import pulp.chips.soft_hier_old.memory as memory
import interco.router as router
import gvsoc.systree
from pulp.chips.soft_hier_old.cluster_registers import ClusterRegisters
...
from pulp.chips.soft_hier_old.snitch.zero_mem import ZeroMem
from elftools.elf.elffile import *
from pulp.chips.soft_hier_old.idma.snitch_dma import SnitchDma
from pulp.chips.soft_hier_old.cluster.l1_interleaver import L1_interleaver
import gvsoc.runner
import math
from pulp.chips.soft_hier_old.snitch.sequencer import Sequencer
...
class ClusterArch:
    def __init__(self,  nb_core_per_cluster, base, cluster_id, tcdm_size,
        ...
        self.barrier_irq            = 19
        self.tcdm                   = ClusterArch.Tcdm(base, self.nb_core + len(spatz_core_list)*spatz_num_vlsu, tcdm_size, nb_tcdm_banks, tcdm_bank_width, sync_itlv, sync_special_mem, tech_node, power_profile)
...
class ClusterTcdm(gvsoc.systree.Component):
    ...
    def __init__(self, parent, name, arch):
        ...
        for i in range(0, nb_banks):
            banks.append(memory.Memory(self, f'bank_{i}', size=arch.bank_size, atomics=True, width_log2=int(math.log2(arch.bank_width)), tech_node=arch.tech_node, power_profile=arch.power_profile))
        ...
        tcdm_sync_mem = memory.Memory(self, 'sync_mem', size=arch.sync_itlv, atomics=True, width_log2=int(math.log2(arch.bank_width)), tech_node=arch.tech_node, power_profile=arch.power_profile)
...
class ClusterUnit(gvsoc.systree.Component):
    ...
    def __init__(self, parent, name, arch, binary, entry=0, auto_fetch=True):
        ...
        #stack memory
        stack_mem = memory.Memory(self, 'stack_mem', size=arch.stack_area.size, tech_node=arch.tech_node, power_profile=arch.power_profile)
        ...
        if arch.multi_idma_enable:
            for x in range(arch.nb_core):
                offload_decoder = SoftHierOldOffloadDecoder(self, f'offload_decoder_{x}', nb_cores=1)
            ...
            offload_decoder = SoftHierOldOffloadDecoder(self, 'offload_decoder', nb_cores=arch.nb_core)
            for core_id in range(0, arch.nb_core):
