...
class SoftHierNetworkInterfaceV2;
 ...
 * or wide).
 */
class SoftHierNetworkQueueV2 : public vp::Block
{
    friend class SoftHierNetworkInterfaceV2;
public:
    SoftHierNetworkQueueV2(SoftHierNetworkInterfaceV2 &ni, std::string name, uint64_t width, int nw);
 ...
 * SoftHierFloonocReqV2 over 'floonoc_link' ports bound to the local (or, for border
 * NIs, nearest) routers of the three physical networks.
 */
class SoftHierNetworkInterfaceV2 : public vp::Component
{
    friend class SoftHierNetworkQueueV2;
...
public:
    ...
    SoftHierNetworkInterfaceV2(vp::ComponentConf &config);

    void reset(bool active);

    void handle_response(SoftHierFloonocReqV2 *req);
    // Recover our own SoftHierFloonocReqV2 from a downstream response beat (initiator-owned
...
private:
    ...
    std::array<SoftHierFloonocLinkMaster, NW_NB> link_out;
    std::array<SoftHierFloonocLinkSlave, NW_NB> link_in;

    vp::Trace trace;
    SoftHierNetworkQueueV2 req_queue;
    SoftHierNetworkQueueV2 wide_queue;
    SoftHierNetworkQueueV2 rsp_queue;
