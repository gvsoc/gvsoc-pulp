...
void SoftHierNetworkQueueV2::reset(bool active)
{
    if (active)
    {
...
void SoftHierNetworkQueueV2::check()
{
...
void SoftHierNetworkQueueV2::unstall()
{
...
void SoftHierNetworkQueueV2::handle_req(vp::IoReq *req, bool wide, SoftHierCollective collective)
{
    ...
    // independent of which internal SoftHierNetworkQueueV2 (req/rsp/wide) carries the
...
void SoftHierNetworkQueueV2::handle_rsp(SoftHierFloonocReqV2 *req, bool is_address)
{
...
void SoftHierNetworkQueueV2::enqueue_router_req(vp::IoReq *req, bool is_address, bool wide, bool is_req, SoftHierCollective collective)
{
    ...
    while(burst_size > 0)
    {
        ...
        // one SoftHierFloonocReqV2 per burst. Data and response phases are paced at
        ...
        // (each beat forwards as a partial response chunk; the SoftHierFloonocReqV2
        ...
        SoftHierEntryV2 *entry = this->ni.get_entry(burst_base, size);
...
void SoftHierNetworkQueueV2::enqueue_router_rsp(SoftHierFloonocReqV2 *req, bool is_address)
{
    // Allocate a fresh SoftHierFloonocReqV2 for the response traversal, copying the
...
void SoftHierNetworkQueueV2::send_router_req()
{
    SoftHierFloonocReqV2 *req = this->queue.front();
    this->queue.pop();
...
SoftHierNetworkInterfaceV2::SoftHierNetworkInterfaceV2(vp::ComponentConf &config)
    : vp::Component(config),
      wide_output_itf(&SoftHierNetworkInterfaceV2::wide_retry, &SoftHierNetworkInterfaceV2::wide_response),
      narrow_output_itf(&SoftHierNetworkInterfaceV2::narrow_retry, &SoftHierNetworkInterfaceV2::narrow_response),
      wide_input_itf(&SoftHierNetworkInterfaceV2::wide_req, &SoftHierNetworkInterfaceV2::wide_response_retry),
      narrow_input_itf(&SoftHierNetworkInterfaceV2::narrow_req, &SoftHierNetworkInterfaceV2::narrow_response_retry),
      link_out{{
        SoftHierFloonocLinkMaster(NW_REQ, &SoftHierNetworkInterfaceV2::link_unstall),
        SoftHierFloonocLinkMaster(NW_RSP, &SoftHierNetworkInterfaceV2::link_unstall),
        SoftHierFloonocLinkMaster(NW_WIDE, &SoftHierNetworkInterfaceV2::link_unstall)
      }},
      link_in{{
        SoftHierFloonocLinkSlave(NW_REQ, &SoftHierNetworkInterfaceV2::link_req),
        SoftHierFloonocLinkSlave(NW_RSP, &SoftHierNetworkInterfaceV2::link_req),
        SoftHierFloonocLinkSlave(NW_WIDE, &SoftHierNetworkInterfaceV2::link_req)
      }},
      fsm_event(this, &SoftHierNetworkInterfaceV2::fsm_handler),
      ...
      response_queue(this, "response_queue", &this->fsm_event)
{
    ...
    this->flit_allocator = SoftHierFloonocReqV2Allocator::get();
    ...
    {
        for (SoftHierEntryV2 &e : this->entries)
        {
            if (e.size == 0)
            {
                continue;
...
SoftHierEntryV2 *SoftHierNetworkInterfaceV2::get_entry(uint64_t base, uint64_t size)
{
    for (int i=0; i<this->entries.size(); i++)
    {
        SoftHierEntryV2 *entry = &this->entries[i];
...
vp::IoRespAck SoftHierNetworkInterfaceV2::wide_response(vp::Block *__this, vp::IoReq *req)
{
...
// Recover our own SoftHierFloonocReqV2 from a downstream response.
//
...
SoftHierFloonocReqV2 *SoftHierNetworkInterfaceV2::unwrap_response(vp::IoReq *req)
{
    SoftHierFloonocReqV2 *self_req = (SoftHierFloonocReqV2 *)req->initiator;
    if (req != self_req)
    {
...
void SoftHierNetworkInterfaceV2::wide_retry(vp::Block *block, vp::IoRetryChannel)
{
    static_cast<SoftHierNetworkInterfaceV2 *>(block)->retry_target(true);
}

void SoftHierNetworkInterfaceV2::retry_target(bool wide)
{
...
bool SoftHierNetworkInterfaceV2::deliver_response(bool wide, vp::IoReq *req, int nw)
{
...
void SoftHierNetworkInterfaceV2::retry_response(bool wide)
{
    auto &pending = this->response_pending[wide];
    vp::IoSlave &port = wide ? this->wide_input_itf : this->narrow_input_itf;
    while (!pending.empty())
...
void SoftHierNetworkInterfaceV2::wide_response_retry(vp::Block *block, vp::IoRetryChannel)
{
    static_cast<SoftHierNetworkInterfaceV2 *>(block)->retry_response(true);
}

void SoftHierNetworkInterfaceV2::narrow_response_retry(vp::Block *block, vp::IoRetryChannel)
{
    static_cast<SoftHierNetworkInterfaceV2 *>(block)->retry_response(false);
}

vp::IoRespAck SoftHierNetworkInterfaceV2::narrow_response(vp::Block *__this, vp::IoReq *req)
{
    SoftHierNetworkInterfaceV2 *_this = (SoftHierNetworkInterfaceV2 *)__this;
    _this->handle_response(_this->unwrap_response(req));
    return vp::IO_RESP_ACCEPTED;
...
void SoftHierNetworkInterfaceV2::narrow_retry(vp::Block *block, vp::IoRetryChannel)
{
    static_cast<SoftHierNetworkInterfaceV2 *>(block)->retry_target(false);
}
    ...
    if (active)
    {
        ...
        for (auto &pending : this->target_pending)
        {
            for (auto held : pending)
            {
                ...
                else this->flit_allocator->free((SoftHierFloonocReqV2 *)held.first);
            }
            pending.clear();
        }
...
int SoftHierNetworkInterfaceV2::get_req_nw(bool is_wide, bool is_write)
{
    if (is_wide)
    {
        return is_write ? SoftHierNetworkInterfaceV2::NW_WIDE : SoftHierNetworkInterfaceV2::NW_REQ;
    }
    else
    {
        return SoftHierNetworkInterfaceV2::NW_REQ;
    }
}

int SoftHierNetworkInterfaceV2::get_rsp_nw(bool is_wide, bool is_write)
{
    return is_wide ? SoftHierNetworkInterfaceV2::NW_WIDE : SoftHierNetworkInterfaceV2::NW_RSP;
}

vp::IoReqStatus SoftHierNetworkInterfaceV2::narrow_req(vp::Block *__this, vp::IoReq *req)
{
    SoftHierNetworkInterfaceV2 *_this = (SoftHierNetworkInterfaceV2 *)__this;
...
vp::IoReqStatus SoftHierNetworkInterfaceV2::wide_req(vp::Block *__this, vp::IoReq *req)
{
    SoftHierNetworkInterfaceV2 *_this = (SoftHierNetworkInterfaceV2 *)__this;
...
vp::IoReqStatus SoftHierNetworkInterfaceV2::handle_req(vp::IoReq *req, bool wide)
{
    ...
    {
        SoftHierEntryV2 *entry = this->get_entry(cursor, remaining);
        if (entry == nullptr)
        {
            mapped = false;
            break;
    ...
    // picks the response port from the inner SoftHierFloonocReqV2's `wide` flag.
...
vp::IoReq *SoftHierNetworkInterfaceV2::make_target_req(SoftHierFloonocReqV2 *req)
{
    req->prepare();
    req->is_first = true;
    req->is_last = true;
    ...
    // to this SoftHierFloonocReqV2; the response callbacks recover it via
...
// target's retry().
bool SoftHierNetworkInterfaceV2::send_to_target(vp::IoReq *to_send, bool wide)
{
    ...
    {
        SoftHierFloonocReqV2 *flit = (SoftHierFloonocReqV2 *)to_send->initiator;
        int64_t latency;
        if (flit != to_send)
        {
...
bool SoftHierNetworkInterfaceV2::link_req(vp::Block *__this, SoftHierFloonocReqV2 *req, int nw)
{
    SoftHierNetworkInterfaceV2 *_this = (SoftHierNetworkInterfaceV2 *)__this;
    ...
    if (req->is_rsp)
    {
        ...
        // The inner SoftHierFloonocReqV2's `wide` flag tells us which external slave
...
void SoftHierNetworkInterfaceV2::link_unstall(vp::Block *__this, int nw)
{
    SoftHierNetworkInterfaceV2 *_this = (SoftHierNetworkInterfaceV2 *)__this;
    SoftHierNetworkQueueV2 *queues[NW_NB] = {&_this->req_queue, &_this->rsp_queue, &_this->wide_queue};
    queues[nw]->unstall();
}


void SoftHierNetworkInterfaceV2::fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    SoftHierNetworkInterfaceV2 *_this = (SoftHierNetworkInterfaceV2 *)__this;
    ...
    {
        _this->handle_response((SoftHierFloonocReqV2 *)_this->response_queue.pop());
    }
...
void SoftHierNetworkInterfaceV2::handle_response(SoftHierFloonocReqV2 *req)
{
    ...
    if (!req->get_is_write())
    {
        ...
        // to the originating NI. handle_rsp allocates a *fresh* SoftHierFloonocReqV2
        ...
        // upstream master when it reaches zero. The SoftHierFloonocReqV2 is only
    ...
    // Keep the SoftHierFloonocReqV2 alive across the beats of a multi-beat resp().
...
{
    return new SoftHierNetworkInterfaceV2(config);
}
