...
from pulp.chips.soft_hier_old.snitch.snitch_isa import *
from pulp.chips.soft_hier_old.snitch.soft_hier_rvv import extend_soft_hier_old_rvv
...
def add_soft_hier_old_includes(isa, redmule: bool=False):
    isa.add_include('<pulp/chips/soft_hier_old/snitch/xdma_legacy.hpp>')
    if redmule:
        isa.add_include('<pulp/chips/soft_hier_old/snitch/redmule_legacy.hpp>')
...
class Snitch(cpu.iss.riscv.RiscvCommon):
    ...
    def __init__(self,
        ...
        isa_key = ("snitch", isa, pulp_v2)
        isa_instance = isa_instances.get(isa_key)

        if isa_instances.get(isa_key) is None:

            if pulp_v2:
                extensions = [ PulpV2(hwloop=False, elw=False), Xf16(), Xf16alt(), Xf8(), XfvecSnitch(), Xfaux() ]
            else:
                extensions = [ Rv32ssr(), Rv32frep(), Xdma(), Rv32redmule(), Xf16(), Xf16alt(), Xf8(), XfvecSnitch(), Xfaux() ]

            isa_instance = cpu.iss.isa_gen.isa_riscv_gen.RiscvIsa("snitch_" + isa, isa,
                no_hash=True, extensions=extensions)
            if not pulp_v2:
                extend_soft_hier_old_rvv(isa_instance)
                add_soft_hier_old_includes(isa_instance, redmule=True)
        ...
        self.add_sources([
            ...
            "pulp/chips/soft_hier_old/snitch/xdma_legacy.cpp",
            "pulp/chips/soft_hier_old/snitch/redmule_legacy.cpp",
        ])
...
class SnitchFast(cpu.iss.riscv.RiscvCommon):
    ...
    def __init__(self,
        ...
        isa_key = ("snitch_fast", isa, inc_spatz, pulp_v2, ssr)
        isa_instance = isa_instances.get(isa_key)
        ...
        if isa_instances.get(isa_key) is None:
            ...
            if pulp_v2:
                ...
                extensions = [ Xdma(), Rv32redmule(), Xf16(), Xf16alt(), Xf8(), XfvecSnitch(), Xfaux() ]
        ...
        self.add_sources([
            "pulp/chips/soft_hier_old/snitch/snitch_fast.cpp",
...
class Snitch_fp_ss(cpu.iss.riscv.RiscvCommon):
    ...
    def __init__(self,
        ...
        isa_key = ("snitch_fp_ss", isa, pulp_v2)
        isa_instance = isa_instances.get(isa_key)
        ...
        if isa_instances.get(isa_key) is None:
            ...
            if not pulp_v2:
                extend_soft_hier_old_rvv(isa_instance)
                add_soft_hier_old_includes(isa_instance)
            add_latencies(isa_instance)
            isa_instances[isa_key] = isa_instance
