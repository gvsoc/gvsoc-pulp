...
void NetworkQueue::handle_req(vp::IoReq *req)
{
    ...
    this->enqueue_router_req(req, true, *(int *)req->arg_get_last(NetworkInterface::REQ_WIDE));
    if (req->get_is_write())
    {
        this->enqueue_router_req(req, false, *(int *)req->arg_get_last(NetworkInterface::REQ_WIDE));
    }
}

void NetworkQueue::handle_rsp(vp::IoReq *req, bool is_address)
{
    this->enqueue_router_req(req, is_address, *(bool *)req->arg_get(FlooNoc::REQ_WIDE));
}
...
void NetworkQueue::enqueue_router_req(vp::IoReq *req, bool is_address, bool wide)
{
