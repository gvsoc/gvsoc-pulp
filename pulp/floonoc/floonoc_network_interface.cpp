...
NetworkQueue::NetworkQueue(NetworkInterface &ni, std::string name,
                           uint64_t width, bool is_wide)
...
// This, respectively the narrow and wide versions, should be called by the
// cluster (or initator of the axi burst)
void NetworkQueue::handle_req(vp::IoReq *req)
{
    this->trace.msg(vp::Trace::LEVEL_DEBUG,
                    "Received %s burst from initiator (burst: %p, offset: "
                    "0x%x, size: 0x%x, "
                    "is_write: %d, op: %d)\n",
                    *(int *)req->arg_get_last(NetworkInterface::REQ_WIDE)
                        ? "wide"
                        : "narrow",
                    req, req->get_addr(), req->get_size(), req->get_is_write(),
                    req->get_opcode());

    this->enqueue_router_req(
        req, true, *(int *)req->arg_get_last(NetworkInterface::REQ_WIDE), true);
    if (req->get_is_write())
    {
        this->enqueue_router_req(
            req, false, *(int *)req->arg_get_last(NetworkInterface::REQ_WIDE),
            true);
    }
}
                                      ...
                                      bool wide, bool is_req)
{
    ...
    {
        uint64_t size =
            is_address ? burst_size : std::min(this->width, burst_size);
        vp::IoReq *router_req = new vp::IoReq();

        // Fill in the information needed by the target network interface to
        ...
        *router_req->arg_get(FlooNoc::REQ_WIDE) = (void *)wide;
        ...
        if (*(NetworkInterface **)req->arg_get(FlooNoc::REQ_SRC_NI))
        {
            ...
            {
                req->get_is_write()
                    ? this->ni.narrow_write_pending_burst_nb_req++
...
void NetworkQueue::send_router_req()
{
    ...
    if (*(NetworkInterface **)req->arg_get(FlooNoc::REQ_SRC_NI))
    {
        ...
        {
            nb_req = burst->get_is_write()
                         ? &this->ni.wide_write_pending_burst_nb_req
                         : &this->ni.wide_read_pending_burst_nb_req;
        }
        else
        {
    ...
    this->trace.msg(vp::Trace::LEVEL_DEBUG,
                    "Handling addr burst (burst: %p, offset: 0x%x, size: 0x%x, "
                    "is_write: %d, op: %d)\n",
                    burst, burst->get_addr(), burst->get_size(),
                    burst->get_is_write(), burst->get_opcode());
...
// This gets called after a request sent to a target was denied, and it is now
// granted
...
{
    this->trace.msg(vp::Trace::LEVEL_DEBUG,
                    "Received request from target (req: %p, base: 0x%x, size: "
                    "0x%x, wide: %d)\n",
                    req, req->get_addr(), req->get_size(),
                    *(int *)req->arg_get_last(NetworkInterface::REQ_WIDE));

    // We use one data in the current burst to store the remaining size and know
    // when the last internal request has been handled to notify the end of
    // burst
...
void NetworkInterface::fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    if (_this->wide_read_pending_burst &&
        _this->wide_read_pending_burst_nb_req == 0)
    {
        _this->wide_read_pending_burst = NULL;
        _this->fsm_event.enqueue();
    }

    if (_this->wide_write_pending_burst &&
        _this->wide_write_pending_burst_nb_req == 0)
    {
        _this->wide_write_pending_burst = NULL;
        _this->fsm_event.enqueue();
    }

    if (_this->narrow_read_pending_burst &&
        _this->narrow_read_pending_burst_nb_req == 0)
    {
        _this->narrow_read_pending_burst = NULL;
        _this->fsm_event.enqueue();
    }

    if (_this->narrow_write_pending_burst &&
        _this->narrow_write_pending_burst_nb_req == 0)
    {
        ...
        _this->nb_pending_bursts[1] < _this->ni_outstanding_reqs)
    {
        _this->trace.msg(vp::Trace::LEVEL_TRACE,
                         "Unstalling denied request (req: %p)\n",
    ...
    if (_this->wide_denied_write_req.size() > 0 &&
        _this->wide_write_pending_burst == NULL &&
        _this->nb_pending_bursts[1] < _this->ni_outstanding_reqs)
    {
        _this->trace.msg(vp::Trace::LEVEL_TRACE,
    ...
    if (_this->narrow_denied_write_req.size() > 0 &&
        _this->narrow_write_pending_burst == NULL &&
        _this->nb_pending_bursts[0] < _this->ni_outstanding_reqs)
    {
        _this->trace.msg(vp::Trace::LEVEL_TRACE,
...
void NetworkQueue::unstall_queue_node(int from_id) {}

bool NetworkQueue::handle_request_node(FloonocNode *node, vp::IoReq *req,
                                       int from_id)
{
    return true;
}
...
void NetworkInterface::unstall_queue_node(int from_id) {}

bool NetworkInterface::handle_request_node(FloonocNode *node, vp::IoReq *req,
                                           int from_id)
{
    return false;
}
