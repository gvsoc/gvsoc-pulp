...
class RouterQueue {
public:
  RouterQueue(vp::Block *parent, std::string name,
 ...
 *
 * Router are the nodes of the noc which are moving internal requests from the
 * network interface to the target.
 */
...
public:
  ...
  // This gets called by other routers or a network interface to move a request
  // to this router
  ...
  // Called by NI to stall the queues in case no more request should be sent to
  // NI
...
private:
  ...
  // Return the source or destination position which corresponds to a source or
  ...
  // This block trace
  vp::Trace trace;
  // Size of the input queues. This limits the number of requests from the same
  // source which can be pending
  int queue_size;
